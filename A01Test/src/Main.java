...
public class Main
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			System.out.println("Next Node:"+(nextPoint+1)+" Length" + minPath + " TotalLength:"+ minPath);
		...
		try
		{
			while(passed.size() != 7)
			{
				...
				minPath += localMin;
				nextPoint = pointer;	
				System.out.println("Next Node:"+(nextPoint+1)+" Length" + localMin + " TotalLength:"+ minPath);
				passed.add(nextPoint);
				if(passed.size() == 7)
				{
					System.out.println("Next Node:"+(initialPoint + 1)+ " Length" + (points[nextPoint][initialPoint]) + " TotalLength:" + (minPath + points[nextPoint][initialPoint]));
				}
			}
		}
		catch (ArrayIndexOutOfBoundsException e)
